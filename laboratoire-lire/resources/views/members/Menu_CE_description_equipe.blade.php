  ...
  <div class="content-wrapper">
	...
	<div class="col-md-8">
              <div class="alert alert-warning alert-dismissible">
                ...
                Veuillez saisir la description d'équipe
              </div>    
           ...
           $equipes = Illuminate\Support\Facades\DB::select('select * from appartenances where id_membre = ?', [Session::get('id_CE')]);

           foreach($equipes as $equipe){

               $idd_equipe = $equipe->id_equipe ;

           }
          
          ?>
        
       <form method="post"  id="description_CE">
              
              @csrf
              
              <textarea class="textarea" placeholder="Place some text here" id="textarea" name="description" 
                          style="width: 100%; height: 200px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;">                    
                          <?php                         
                        
                        

                        $descriptions  = Illuminate\Support\Facades\DB::select('select description from equipes where id = ?', [$idd_equipe]);  
                        ...
                        <div class="box-footer col-xs-12">       
                            <input type="submit" class="btn btn-success pull-right col-xs-12" value="confirmer" id="send" >         
                        </div>
           </form>         
        </div>
   <div class="col-md-2"></div>
...
//https://stackoverflow.com/questions/14393512/tinymce-textarea-and-post-form-using-ajax?utm_medium=organic&utm_source=google_rich_qa&utm_campaign=google_rich_qa
       ...
       var data = new FormData(form)
           ...
           //$("#loader").show();
            ...
            $.ajax({
                    type: "POST",
                    url: "{{ route('Chef_Equipe.Description') }}",
                    data: data  ,
                    processData: false,    
                    contentType: false,
                    //cache: false,
